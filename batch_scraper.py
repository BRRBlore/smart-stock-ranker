...
SCHEMA_COLS = set(KEY_MAP.values()) | {
    ...
    "composite_score","grade","rank",
    "score_value","score_quality","score_momentum","score_smartmoney",
    "rank_value","rank_quality","rank_momentum","rank_smartmoney",
    "fair_value","buy_zone_low","buy_zone_high",
    "strong_buy_below","value_signal","valuation_methods",
}
    ...
    for i, meta in enumerate(tickers_meta):
        ...
        try:
            ...
            if result:
                upsert_stock(result)
                completed += 1
                log.info(f"  ✓  Score={result.get('composite_score',0):.0f}  "
...
if __name__ == "__main__":
    ...
    if force:
        print("FORCE_SCRAPE=true — re-scraping all stocks...")
        from database import get_universe
        pending = get_universe().to_dict(orient="records")
    else:
        pending = get_pending_tickers()
    ...
    if not pending:
        print("Nothing to scrape. Run universe_builder.py or auto_universe.py first.")
    else:
        ...
        for i, meta in enumerate(pending):
            ...
            try:
                ...
                if result:
                    upsert_stock(result)
                    completed += 1
                    print(f"  ✓  Score={result.get('composite_score',0):.0f}  "
                          f"Signal={result.get('value_signal','—')}")
                else:
                    print("  ✗  No data")
